...
func promptForToken(t testing.TB, ctx context.Context, oauthConfig OAuthConfig) string {
	callbackPath, err := devenv.ResolvePath("<dev_state>/tests/powerservice_scrape/callback_url")
	if err != nil {
		t.Fatal(err)
	}
...
func getToken(t testing.TB, ctx context.Context, oauthConfig OAuthConfig) string {
	tokenPath, err := devenv.ResolvePath("<dev_state>/tests/powerservice_scrape/token")
	if err != nil {
		t.Fatal(err)
	}
...
func TestScrape(t *testing.T) {
	cleanup := telemetry.SetupForTesting("test:powerservice")
	defer cleanup()
	...
	require.NotEmpty(t, res.GetProfile().GetFirstName())
	require.NotEmpty(t, res.GetProfile().GetLastName())
	...
	for _, course := range courses {
		...
		require.NotEmpty(t, course.GetTeacherFirstName())
		require.NotEmpty(t, course.GetTeacherLastName())

		if course.GetRoom() == "" {
			slog.Warn("no room found", "course", course.GetName())
		}

		if len(course.GetTerms()) == 0 {
			slog.Warn("no terms present", "course", course.GetName())
		}
		for _, term := range course.GetTerms() {
			require.NotEmpty(t, term.GetStart())
			require.NotEmpty(t, term.GetEnd())
		}
